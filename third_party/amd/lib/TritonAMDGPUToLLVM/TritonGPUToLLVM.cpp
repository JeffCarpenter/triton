...
void populateTritonGPUToLLVMPatterns(
  ...
  patterns.add<ExtractSliceOpConversion>(typeConverter, moduleAllocation,
                                         benefit);
  patterns.add<GetProgramIdOpConversion>(typeConverter, benefit);   // no
  patterns.add<GetNumProgramsOpConversion>(typeConverter, benefit); // no
  mlir::triton::common::populateMakeRangeOpToLLVMPattern(typeConverter,
                                                         patterns, benefit);
  patterns.add<ReturnOpConversion>(typeConverter, benefit); // no
  patterns.add<PrintOpConversion>(
      typeConverter, benefit); // maybe, but it uses LLVM::NVIDIA::llGetPid
